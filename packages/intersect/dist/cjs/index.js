...
__export(exports, {
  ...
  intersectArcLineSegment: () => intersectArcLineSegment,
  intersectArcRectangle: () => intersectArcRectangle,
  intersectBoundsArc: () => intersectBoundsArc,
  intersectBoundsBounds: () => intersectBoundsBounds,
  intersectBoundsCircle: () => intersectBoundsCircle,
  intersectBoundsEllipse: () => intersectBoundsEllipse,
  intersectBoundsLineSegment: () => intersectBoundsLineSegment,
  intersectBoundsPolygon: () => intersectBoundsPolygon,
  intersectBoundsPolyline: () => intersectBoundsPolyline,
  intersectBoundsRay: () => intersectBoundsRay,
  intersectBoundsRectangle: () => intersectBoundsRectangle,
  intersectCircleBounds: () => intersectCircleBounds,
  intersectCircleCircle: () => intersectCircleCircle,
  intersectCircleLineSegment: () => intersectCircleLineSegment,
  intersectCircleRectangle: () => intersectCircleRectangle,
  intersectEllipseBounds: () => intersectEllipseBounds,
  intersectEllipseCircle: () => intersectEllipseCircle,
  intersectEllipseEllipse: () => intersectEllipseEllipse,
  intersectEllipseLineSegment: () => intersectEllipseLineSegment,
  intersectEllipseRay: () => intersectEllipseRay,
  intersectEllipseRectangle: () => intersectEllipseRectangle,
  intersectLineSegmentArc: () => intersectLineSegmentArc,
  ...
  intersectLineSegmentRay: () => intersectLineSegmentRay,
  intersectLineSegmentRectangle: () => intersectLineSegmentRectangle,
  intersectPolygonBounds: () => intersectPolygonBounds,
  intersectPolygonLineSegment: () => intersectPolygonLineSegment,
  intersectPolygonRectangle: () => intersectPolygonRectangle,
  intersectPolylineBounds: () => intersectPolylineBounds,
  intersectPolylineLineSegment: () => intersectPolylineLineSegment,
  intersectPolylineRectangle: () => intersectPolylineRectangle,
  intersectRayBounds: () => intersectRayBounds,
  intersectRayEllipse: () => intersectRayEllipse,
  intersectRayLineSegment: () => intersectRayLineSegment,
  intersectRayRay: () => intersectRayRay,
  intersectRayRectangle: () => intersectRayRectangle,
  intersectRectangleArc: () => intersectRectangleArc,
  intersectRectangleBounds: () => intersectRectangleBounds,
  intersectRectangleCircle: () => intersectRectangleCircle,
  intersectRectangleEllipse: () => intersectRectangleEllipse,
  intersectRectangleLineSegment: () => intersectRectangleLineSegment,
  intersectRectanglePolygon: () => intersectRectanglePolygon,
  intersectRectanglePolyline: () => intersectRectanglePolyline,
  intersectRectangleRay: () => intersectRectangleRay,
  intersectRectangleRectangle: () => intersectRectangleRectangle
});
...
function intersectRayEllipse(origin, direction, center, rx, ry, rotation) {
  const a1 = origin;
  const a2 = import_vec.Vec.mul(direction, 999999999);
...
function intersectLineSegmentLineSegment(a1, a2, b1, b2) {
  const AB = import_vec.Vec.sub(a1, b1);
  const BV = import_vec.Vec.sub(b2, b1);
  const AV = import_vec.Vec.sub(a2, a1);
  ...
  if (u_b !== 0) {
    ...
    if (0 <= ua && ua <= 1 && 0 <= ub && ub <= 1) {
      return createIntersection("intersection", import_vec.Vec.add(a1, import_vec.Vec.mul(AV, ua)));
    }
  }
  return createIntersection("no intersection");
}
...
function intersectLineSegmentArc(a1, a2, center, radius, start, end) {
  const sa = import_vec.Vec.angle(center, start);
  const ea = import_vec.Vec.angle(center, end);
  const ellipseTest = intersectEllipseLineSegment(center, radius, radius, 0, a1, a2);
  if (!ellipseTest.didIntersect)
    return createIntersection("no intersection");
  const points = ellipseTest.points.filter((point) => isAngleBetween(sa, ea, import_vec.Vec.angle(center, point)));
  if (points.length === 0) {
    return createIntersection("no intersection");
  }
...
function intersectLineSegmentCircle(a1, a2, c, r) {
  ...
  const results = [];
  if (0 <= u1 && u1 <= 1)
    results.push(import_vec.Vec.lrp(a1, a2, u1));
  if (0 <= u2 && u2 <= 1)
    results.push(import_vec.Vec.lrp(a1, a2, u2));
  return createIntersection("intersection", ...results);
}
function intersectLineSegmentEllipse(a1, a2, center, rx, ry, rotation = 0) {
  if (rx === 0 || ry === 0 || import_vec.Vec.isEqual(a1, a2)) {
    return createIntersection("no intersection");
  }
  rx = rx < 0 ? rx : -rx;
  ry = ry < 0 ? ry : -ry;
  a1 = import_vec.Vec.sub(import_vec.Vec.rotWith(a1, center, -rotation), center);
  a2 = import_vec.Vec.sub(import_vec.Vec.rotWith(a2, center, -rotation), center);
  const diff = import_vec.Vec.sub(a2, a1);
  ...
  const points = tValues.filter((t) => t >= 0 && t <= 1).map((t) => import_vec.Vec.add(center, import_vec.Vec.add(a1, import_vec.Vec.mul(import_vec.Vec.sub(a2, a1), t)))).map((p) => import_vec.Vec.rotWith(p, center, rotation));
  return createIntersection("intersection", ...points);
}
